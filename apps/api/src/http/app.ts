...
export type AppDeps = { recorded?: RecordedStreams; agent?: CreativeAgent; brandAgent?: BrandAgent; artifacts?: ArtifactStore };

// Mock mode sets `recorded`; routes without their agent answer 501.
export function createApp(config: Config, { recorded, agent, brandAgent, artifacts }: AppDeps = {}) {
  const app = new Hono();
  ...
  app.route("/v1/brands", brandRoutes(config, recorded, brandAgent));
